...
def _init_loggper(logger_name,level):
    ...
    os.makedirs(os.path.join(current_directory,'TSU-BGPMonitor-Consumer', LOGS_DIR, logger_name), exist_ok=True)
    handler = ConcurrentRotatingFileHandler(os.path.join(current_directory,'TSU-BGPMonitor-Consumer',LOGS_DIR,logger_name,f'{logger_name}.log'), encoding='UTF-8',maxBytes=5*1024*1024,backupCount=BACKUP_COUNT)
